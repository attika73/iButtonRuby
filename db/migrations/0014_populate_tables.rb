...
class PopulateTables < Sequel::Migration
  ...
  def up
    ...
    DB[:prs_owners].insert(nil,"Unknown","Owner",Time.now(),Time.now())
    DB[:prs_owners].insert(nil,"Attila","Albert",Time.now(),Time.now())
    ...
    DB[:prs_admins].insert(nil,2,"evoline","Albert.Attila@evoline.ro",nil,nil,Time.now(),Time.now())
